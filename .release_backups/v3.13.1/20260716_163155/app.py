...
def render_beta_access() -> None:
    ...
    if current_key:
        ...
        if is_cloud_store_configured():
            st.sidebar.caption("持股與觀察清單會保存到雲端。")
        else:
            st.sidebar.warning("尚未設定雲端保存，資料只會暫存在本次瀏覽。")
    ...
    if st.sidebar.button("載入 / 保存我的資料"):
        if not email.strip() or not code.strip():
            ...
            st.session_state["active_page"] = "我的持股"
            st.rerun()
    st.sidebar.caption(f"資料保存狀態：{cloud['status']}")
    if not is_cloud_store_configured():
        st.sidebar.info("未設定雲端保存時仍可體驗，但重新開瀏覽器後資料不會永久保存。")
...
<div class='ux-hero'>
  ...
  <div class='ux-hero-text'>今天只做有價格條件的低接或突破確認，盤中急拉不追。先處理持股風險，再看今日可操作清單。</div>
</div>
""",
        unsafe_allow_html=True,
    )
...
def _render_kpis(payload: dict) -> None:
    ...
    cards = [
        ("今日可操作", len(payload.get("buy_list", [])), "已符合條件，可檢查進場區與風險線"),
        ("強勢觀察", len(strength.get("strong_list", [])), "趨勢強，但需突破或回檔確認"),
        ("等待條件", len(payload.get("wait_list", [])), "沒到價不做，避免追高"),
        ("避開/控風險", len(payload.get("avoid_list", [])), "風險偏高，先不新增部位"),
    ]
    cols = st.columns(4)
    for col, (label, value, hint) in zip(cols, cards):
        col.markdown(
            f"<div class='ux-kpi'><span>{label}</span><b>{value}</b><em>{hint}</em></div>",
            unsafe_allow_html=True,
        )


def _main_decision_copy() -> list[tuple[str, str]]:
    return [
        ("先處理持股風險", "持股若跌破風險線，先減碼；沒有跌破就續抱觀察。"),
        ("只看已符合條件的標的", "今日可操作清單內的股票，才進一步看進場區、停損線與理由。"),
        ("不追盤中急拉", "沒有進入買進區、法人籌碼不足、或量價結構轉弱的標的，今天先不碰。"),
    ]


def _render_today_action_plan() -> None:
    st.subheader("今天先做這 3 件事")
    for idx, (title, body) in enumerate(_main_decision_copy(), start=1):
        st.markdown(
            f"""
<div class='ux-action-step'>
  <b>{idx}. {title}</b>
  <div>{body}</div>
</div>
""",
...
def _decision_table_rows(payload: dict) -> list[dict]:
    rows: list[dict] = []
    buckets = [
        ("今日可操作", payload.get("buy_list", [])[:4]),
        ("等待條件", payload.get("wait_list", [])[:4]),
        ("控風險", payload.get("avoid_list", [])[:4]),
    ]
    for category, cards in buckets:
        for card in cards:
            rows.append({
                "優先級": len(rows) + 1,
                "股票": card.get("label", "—"),
                "分類": category,
                "今日建議": card.get("setup") or card.get("grade") or "—",
                "觸發條件": _format_key_prices(card),
                "風險線 / 下一步": _compact_next_step(card),
            })
    return rows


def _render_today_priority_table(payload: dict) -> None:
    rows = _decision_table_rows(payload)
    st.subheader("今日優先清單")
    if not rows:
        st.info("目前沒有符合條件的標的。今天以等待為主，不追高。")
        return
    st.dataframe(rows, use_container_width=True, hide_index=True)
...
<style>
    ...
    .ux-kpi em { display:block; margin-top:8px; color:#64748B; font-size:0.84rem; font-style:normal; line-height:1.45; }
    .ux-action-step { border:1px solid #E5E7EB; border-radius:16px; padding:14px 16px; background:#FFFFFF; margin-bottom:10px; }
    .ux-action-step b { color:#0F172A; }
    .ux-action-step div { color:#475569; line-height:1.6; margin-top:4px; }
...
if st.button("更新今日資料", use_container_width=False):
    with st.spinner("股市老師更新今日策略中..."):
...
if page == "今天怎麼做":
    st.header("今天怎麼做")
    st.markdown(f"<div class='ux-callout'>今日判斷：{payload.get('market_view', '今日策略待產生')}</div>", unsafe_allow_html=True)
    _render_today_action_plan()
    _render_today_priority_table(payload)
