...
def main() -> None:
    ...
    print(f"資料來源：官方採用 {source.get('official_confirmed', 0)} / Yahoo採用 {source.get('yahoo_selected', source.get('yahoo_newer_than_official', 0) + source.get('yahoo_only', 0))} / 官方異常 {source.get('official_anomaly', 0)} / Fallback {source.get('fallback', 0)}")
    print("今日可買進：")
