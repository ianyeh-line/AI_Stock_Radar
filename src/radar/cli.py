...
def run() -> None:
    ...
    print("🚀 AI Stock Radar v1.0.0")
    print("Investment Manager Release")
    ...
    print(f"PM 早會：{brief['headline']}")
    print(f"資金配置：{brief['capital_allocation']}")
    print("波段操作 Top 5:")
    for idx, card in enumerate(payload["decision_cards"][:5], 1):
        print(f"  {idx}. {card['display_name']} | {card['radar_score']} | {card['decision']} | {card['conviction']} | {card['confidence']}%")
